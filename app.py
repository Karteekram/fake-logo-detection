...
def is_likely_logo(image):
    ...
    photo_score = 0
    photo_score += 1 if contrast > 55 else 0
    photo_score += 1 if channel_std > 50 else 0
    photo_score += 1 if edge_strength < 28 else 0
    photo_score += 1 if color_diversity_ratio > 0.45 else 0
    photo_score += 1 if bg_ratio < 0.22 else 0
    photo_score += 1 if skin_ratio > 0.12 else 0
    photo_score += 1 if high_texture_ratio > 0.35 else 0

    looks_like_photo = photo_score >= 4
